        ...
        .then(data => {
            ...
            data.resultado.forEach(empleado => {
                ...
                celdaAcciones.innerHTML = `
                    <button onclick="editarEmpleado(${empleado.id})" class="btn btn-primary btn-sm m-1">Editar</button>
                    <button onclick="eliminarEmpleado(${empleado.id})" class="btn btn-danger btn-sm m-1">Eliminar</button>
                `;
...
const editarBeneficiario = (id) => {
    ...
    fetch(`https://fastapi-cci.onrender.com/buscarbeneficiario/${id}`)
        ...
        .catch(error => {
            ...
            alert("Hubo un error al cargar los datos del beneficiario.");
        });
};
